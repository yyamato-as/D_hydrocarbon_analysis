...
source = "MWC_480"
species = ["C2H"]
lines = ["C2H_1-0_fs1", "C2H_3-2_fs1", "C2H_3-2_fs2"]
...
fitdir = f"./data/fit/{source}/C2H_C2D_LTE_noconv/"
...
def calc_tau(nu, paramset):
    ...
    for s in species:
        ...
        sigma_nu = au.dv2dnu_abs(dv=paramset.sigma.value, nu0=data.nu0)
...
def calc_full_model(paramset):
    ...
    for line, nu in zip(lines, nu_list):
        ...
        # intensity
        I = (
            specmodel.Bnu(nu, paramset.Tex.value)
            - specmodel.Bnu_CMB(nu)
            - I_cont
        ) * (1 - np.exp(-tau_l))

        # correct for the spectral response function
        # I = au.convolve_boxcar(x=nu, y=I, w=native_dchan)
        # I_cont = au.convolve_boxcar(x=nu, y=I_cont, w=native_dchan)
