...
def calc_sensitivity(
    ...
    for vis in msfile:
        ...
        print(sens)
        print(f"# Briggs sensitivity with robust = {robust}: {sens[0]} Jy")
        print(f"# Relative to natural weighting: {sens[1]}")
        sensitivities.append(sens[0])
    avesens = np.mean(sensitivities) / len(sensitivities) ** 0.5
    print(
        f"### Averaged sensitivity over {len(sensitivities)} measurement sets: {sens} Jy"
    )
    return avesens
...
def finalize_selfcal(
    ...
    for i in data_params.keys():
        ...
        # update data_params to the version of final iteration
        data_params[i][visID + "_selfcal"] = outputvis
        for key in data_params_selfcal_keys:
            data_params[i]["selfcal_" + key] = data_params[i]["selfcal_" + key][:iteration]
